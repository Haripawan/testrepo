...
# Step 5: Analyze for PII, calculate average confidence per column, and add sample value
def analyze_pii(dataframe, table_name):
    pii_report = []

    for column in dataframe.columns:
        # Dictionary to hold confidence scores and sample values for each detected PII type
        pii_scores = defaultdict(list)  # {PII_type: [list of confidence scores]}
        pii_samples = {}  # {PII_type: sample_value} - store one example of each PII type
        ...
        for value in dataframe[column].astype(str):
            ...
            for result in results:
                ...
                # Store the first sample value if not already recorded for this PII type
                if result.entity_type not in pii_samples:
                    pii_samples[result.entity_type] = value
        ...
        if pii_scores:
            ...
            best_pii_type = max(avg_confidences, key=avg_confidences.get)
            best_avg_confidence = avg_confidences[best_pii_type]
            sample_value = pii_samples[best_pii_type]  # The sample value for the best PII type
            ...
            pii_report.append({
                ...
                "Detected PII Type": best_pii_type,
                "Average Confidence Score": best_avg_confidence,
                "Sample Value": sample_value  # Adding the sample value to the report
            })
